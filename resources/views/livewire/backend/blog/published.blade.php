        ...
        <div class="col-xl-12 col-md-12 col-lg-12 col-12">
            ...
            <div class="table-responsive">
                ...
                <tbody>
                    ...
                    <tr class="@if ($this->isChecked($item->id)) table-primary @endif">
                        ...
                        <td class="text-center align-midle">
                            ...
                            <button wire:click="selectItem('{{ $item->id }}', 'primary')"
                                ...
                                <button wire:click="selectItem('{{ $item->id }}', 'selection')"
                                    ...
                                    <button wire:click="selectItem('{{ $item->id }}', 'inactiveh')"
                                        ...
                                        <button wire:click="selectItem('{{ $item->id }}', 'activeh')"
                                            ...
                                            href="{{ route('backend.blog.edit', $item->id) }}"><i
                                            class="fa fa-edit"></i></a>
                                            <button wire:click="selectItem('{{ $item->id }}' , 'deletepublised')"
                                                class="mx-1 my-1 btn btn-xs btn-danger" title="Send to Trash"><i
                                ...
                                @else
                                <hr>
                                <h2 style="color: red" class="text-center">Blog Published not available</h2>
                                @endif
                            </div>
                        </div>
                        <div class="modal center-modal fade" id="modalFormDeletepublished" tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Blog Published Send to Trash Item</h5>
                    ...
                    @push('scripts')
                    <script>
                        window.addEventListener('openDeleteModalpublished', event => {
                            $("#modalFormDeletepublished").modal('show');
                        });

                        window.addEventListener('closeDeleteModalpublished', event => {
                            $("#modalFormDeletepublished").modal('hide');
                        });
                    </script>
                    @endpush

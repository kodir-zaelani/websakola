...
class BlogController extends Controller
{
    ...
    */
    ...
    {
        return [
            'permission:blog.index|blog.create|blog.edit|blog.delete|blog.trash',
        ];
    }
    ...
    public function store(RequestPostStore $request)
    {
        ...
        if ($request->has('image')) {
            ...
            $fileName = 'post_' . Str::slug($request->title) . '.' . $extension;
        ...
        if ($post) {
            ...
            return redirect()->route('backend.blog.index')->with(['success' => 'Add Post ' . $post['title'] . ' was successfully!']);
        } else {
            //redirect dengan pesan error
            return redirect()->route('backend.blog.index')->with(['error' => 'Data Gagal Disimpan!']);
        }
    }
    ...
    */
    public function update(RequestPostUpdate $request, Post $post)
    {
        ...
        if ($post) {
            //redirect dengan pesan sukses
            return redirect()->route('backend.blog.index')->with(['warning' => 'Update Blog ' . $post['title'] . ' was successfully!']);
        } else {
            //redirect dengan pesan error
            return redirect()->route('backend.blog.index')->with(['error' => 'Data Gagal Diperbaharui!']);
        }
    }
